...
function getNextFriday(date) {
  ...
  nextDate.setDate(
    date.getDate() +
      ((12 - date.getDay()) % 7 === 0 ? 7 : (12 - date.getDay()) % 7)
  );
  return nextDate;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const res = new Date(dateEnd) - new Date(dateStart);
  return Math.floor(res / (1000 * 60 * 60 * 24)) + 1;
}
...
function formatDate(date) {
  ...
  const h12 = hh % 12 ? hh % 12 : 12;
  return `${m}/${d}/${yyyy}, ${h12}:${mm}:${ss} ${amPm}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 0; i < new Date(year, month, 0).getDate(); i += 1) {
    if (dateObj.getDay() === 0 || dateObj.getDay() === 6) res += 1;
...
function getWeekNumberByDate(date) {
  ...
  const countDay = (date - startDay) / (60 * 60 * 24 * 1000);
  const res = Math.ceil((countDay + startDay.getDay() + 1) / 7);
  return res;
}
...
function getNextFridayThe13th(date) {
  ...
  for (let i = 0; i < 12; i += 1) {
    if (date.getDay() === 5) return new Date(date);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  for (let i = firstDay; i <= lastDay; i.setDate(i.getDate() + 1)) {
    if (workDays < countWorkDays) {
      scheduleArr.push(
        `${i.getDate().toString().padStart(2, 0)}-${(i.getMonth() + 1).toString().padStart(2, 0)}-${i.getFullYear().toString().padStart(2, 0)}`
      );
      workDays += 1;
      if (workDays === countWorkDays) offDays = 0;
    } else if (offDays < countOffDays) {
      offDays += 1;
      if (offDays === countOffDays) workDays = 0;
    }
  }
  return scheduleArr;
}
